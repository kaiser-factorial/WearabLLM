...
static esp_err_t init_playback(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_codec_dev_set_out_vol(s_play_dev, CONFIG_WEARABLLM_AUDIO_OUT_VOLUME),
                        TAG,
...
esp_err_t wearabllm_audio_init(void)
{
    ...
    for (int i = 0; i < 4; i++) {
        esp_codec_dev_set_in_channel_gain(s_record_dev, ESP_CODEC_DEV_MAKE_CHANNEL_MASK(i), RECORD_VOLUME);
    }

    s_audio_ready = true;
    ESP_LOGI(TAG, "ES7210 microphone capture ready");
#if CONFIG_WEARABLLM_AUDIO_OUT_ENABLED
    ...
    size_t *out_len)
{
    ...
    while (pcm_bytes + (I2S_READ_FRAMES * sizeof(int16_t)) <= max_pcm_bytes) {
        ...
        for (int i = 0; i < I2S_READ_FRAMES; i++) {
            int32_t left = i2s_samples[i * 2 + 0];
            int32_t right = i2s_samples[i * 2 + 1];
            int32_t mono = (left / 2) + (right / 2);
            pcm_out[i] = (int16_t)(mono >> 16);
        }
